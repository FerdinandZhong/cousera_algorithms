...
class MedianMaintainer:
    ...
    def _add_number(self, num):
        ...
        # print(self._heap_low, self._heap_high)
        self._median_sum += self._heap_low.peek() if len(self._heap_low) >= len(
            self._heap_high) else self._heap_high.peek()
        print(self._heap_low.peek() if len(self._heap_low) >= len(
            self._heap_high) else self._heap_high.peek())

if __name__ == "__main__":
    BASE_DIR = os.path.join(os.path.dirname(__file__))
    median_maintainer = MedianMaintainer(input_file=BASE_DIR + '/data/Median.txt')
